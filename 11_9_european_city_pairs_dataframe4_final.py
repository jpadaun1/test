...
for city_pair_eng in city_pairs_eng:
   ...
   #this_data=[origin,destination,metres_distance,secs_travel_time]
   #uk_city_pairs = pd.DataFrame(data_dict)
    #rint(uk_city_pairs)

city_pairs_eng=pd.DataFrame(combined_list,columns=list_labels)
print(city_pairs_eng)
city_pairs_eng.to_csv('example.csv', index=True, header=True, sep=',')
...
#df.to_csv('example.csv', index=True, header=True, sep=',')
